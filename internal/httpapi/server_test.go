...
func TestHandleClientLogsAccepted(t *testing.T) {
	...
	server := NewServer(logging.NewBootstrapLogger(&logBuffer), nil, nil, nil, nil, nil, nil)
...
func TestHandleClientLogsRejectsInvalidLog(t *testing.T) {
	server := NewServer(logging.NewBootstrapLogger(&bytes.Buffer{}), nil, nil, nil, nil, nil, nil)
