...
- `web/src/App.tsx` 现在把 `H5` 入会预览落成了预定会议的真实流程节点；快速会议和加入会议会直接进入会议，同时首页补上最近会议摘要，移动端预览页按手机 / `iPad` 的信息密度重新排布。
...
- 修复 `H5` 移动端加入会议扫码弹窗底部按钮被浏览器视口裁切的问题；扫码弹窗在小屏上会靠上显示并压缩内部尺寸。
- 优化加入会议扫码识别体验：实时扫码优先使用浏览器原生二维码识别能力，回退时缩小扫描区域并限制处理频率，同时分享二维码改为更易识别的深色码点浅色底样式。
- 修复 `H5` 移动端无法稳定显示主持人共享画面的问题：远端视频现在按移动端浏览器策略静音自动播放，远端声音由独立音频节点承载。
- 修复 `Web` 桌面端与 `H5` 移动端登录、加入会议等入口页在小屏设备上需要上下拖动才能看到底部按钮的问题；入口卡片现在会压缩表单密度并保持单页显示。
- 修复会议结束或退出后的会议号缓存策略：主持人结束会议后其他参会者会清空缓存会议号，主动离会后再次进入加入页会先校验上一次会议是否仍在进行中。
- 修复 `H5` 移动端会中首屏布局：进入会议后默认保持单页主舞台，聊天和成员列表默认隐藏，点击后贴着底部工具栏上方弹出，避免面板出现在屏幕中央。
- 修复 H5 全屏后仍可滚动以及底部 5 个工具按钮在窄屏互相重叠的问题；全屏态现在锁定单页高度，移动端工具栏按钮会按屏幕宽度动态收缩。
...
- `H5` 移动端进入会议后的真实会中页已按现有设计稿重新收口，顶部信息、主舞台、底部工具栏以及成员 / 聊天 / 应用面板的布局现在与 `docs/design/h5-room-preview.*` 和 `docs/design/h5-ui-spec.md` 保持一致。
