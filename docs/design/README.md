...
- `h5-auth-preview.html`：`H5` 登录 / 注册 / 登录后入口 / 预定会议 / 加入会议 / 扫码加入 / 密码确认 / 入会预览
...
- `wechat-room-preview.html`：`微信小程序` 会中壳层、视频区域占位和当前成员 / 聊天 / 更多面板预览
...
- `h5-auth-preview.html` 顶部按钮可切换手机 / `iPad` 视图，以及 `H5` 登录、注册、首页、预定会议、加入会议、扫码加入、密码确认和入会预览状态。
- `h5-room-preview.html` 顶部按钮可切换手机 / `iPad`、主画面 / 头像墙和聊天 / 成员 / 应用 / 邀请等面板状态。
- `wechat-auth-preview.html` 顶部按钮可切换手机 / `Pad` 视图，以及微信小程序当前已实现的登录、首页、加入会议、密码确认和入会预览状态。
- `wechat-room-preview.html` 顶部按钮可切换手机 / `Pad`、舞台视图 / 会议概览和成员 / 聊天 / 更多面板。
