...
- 让 `update.sh` 保持就地更新风格，完成下载、校验、覆盖、重启三个容器的一键更新；首次部署时应能识别空目录并跳过停止旧栈。
...
3. **coturn**
   ...
   - relay 端口范围默认改为 `52000-52048`，并允许在打包时覆盖，避免宿主机高位端口冲突导致整个容器无法启动。
...
- 更新 `current` 版本标记文件。
- 重启三个 Docker 容器。
- 若当前目录尚未存在上一版完整部署，应跳过停止旧栈步骤，直接完成首次解压和启动。
- 失败时必须记录失败步骤和命令，避免日志只停留在“开始执行某一步”。
- 不使用临时目录。
- 不备份旧版本。
- 不做自动回滚。
- 不管理 Nginx。
...
- `docker-compose.yml`
  ...
  - coturn relay 端口范围默认使用 `52000-52048`
...
- 若 `update.sh` 缺少步骤级失败日志，线上排障成本会显著升高，因此错误日志必须包含至少：步骤、退出码和失败命令。
